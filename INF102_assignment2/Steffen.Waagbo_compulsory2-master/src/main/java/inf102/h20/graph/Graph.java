...
import inf102.h20.iterating.IterableIterator;
...
public class Graph<V> implements IGraph<V> {
	...
	public Iterable<V> vertices() {
		//This line protects the HashSet vertices from being modified
		//If it was possible to remove vertices from the graph by using
		//the iterator we would not be able to ensure that edges was removed at the same time.
		return IterableIterator.wrap(this.vertices);
		
		//simpler way to solve this problem but this costs time and memory
		//return (Iterable<V>) this.vertices.clone();
		
		//TODO: write an VertexIterator such that remove method will actually
		//call the removeVertex method in the graph?
	}
	...
	public Iterable<V> neighbours(V v) {
		return IterableIterator.wrap(getNeighbours(v));
	}
